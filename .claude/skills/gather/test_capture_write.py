...
r = run("write", "--url", "https://x.test/one", "--ledger-id", RID,
        "--engine", "defuddle", "--title", "One", stdin="# One\n\nbody [[stray]]\n")
...
r = run("write", "--url", "https://x.test/one", "--ledger-id", RID, "--title", "One", stdin="x")
check("existing path refused",   r.returncode == 2 and "immutable" in r.stderr)
r = run("write", "--url", "https://x.test/two", "--ledger-id", RID, "--title", "Two", stdin="y")
check("second write ok",         r.returncode == 0)
r = run("write", "--url", "https://x.test/three", "--ledger-id", RID, "--title", "Three", stdin="z")
check("budget spent refused",    r.returncode == 2 and "budget spent" in r.stderr)
r = run("write", "--url", "https://x.test/four", "--ledger-id", "missing-" + RID, stdin="w")
