...
- With a bigger budget (say `--max-pages 40`) the shortlist stays small but every further ranked candidate is listed one per line, and you capture by **rule** — *"top 30 by relevance"*, with row-level tweaks if you like. A bare "approve" always means just the shortlist, and the search funnel (queries, pool, spot-checks) scales with the budget and is printed at the gate.
...
- **Roles**: say *"act as tutor"* (or any role you define under `## Roles`) to switch the agent's task context for the conversation; every reply opens with the active `role · style`. Roles shape how the agent works on your task and add to your global preferences — a role replaces one only where you mark it with `overrides`.
