...
- **Throttle**: subagent model and effort tiers follow one knob, `throttle` in `CUSTOMISATION.md` (`top · default · cheap · fast · cheap-fast`). Say *"set throttle to fast"* and the agent runs `throttle.py set fast`, which rewrites the Settings line and every subagent definition; the change registers at your next message and, on a machine with the anchor hook installed, the status anchor shows it. `default` keeps every lane at maximum effort with the model chosen per task; `cheap`/`fast` are you naming a budget or time constraint. Which steps run in lanes at all is fixed per skill by parity gates (`wiki/developments/fable-minimising-routing.md`).
